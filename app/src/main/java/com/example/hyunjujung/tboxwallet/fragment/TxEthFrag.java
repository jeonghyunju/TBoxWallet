...
public class TxEthFrag extends Fragment {
    private static final String TAG = "TxEthFrag";
    ...
    RecyclerView TxEthRecycle;
    LinearLayoutManager txEthLM;
    ArrayList<TransactionData> txEthArray = new ArrayList<>();
    TransactionAdapter txAdapter;

    TextView noTxEthList;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.txeth_frag, container, false);

        TxEthRecycle = view.findViewById(R.id.TxEthRecycle);
        noTxEthList = view.findViewById(R.id.noTxEthList);
        txEthLM = new LinearLayoutManager(getContext());
        TxEthRecycle.setLayoutManager(txEthLM);
        ...
        /* 리사이클러뷰 아이템터치리스너 */
        TxEthRecycle.addOnItemTouchListener(new RecyclerView.OnItemTouchListener() {
            @Override
            public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent e) {
                View childView = rv.findChildViewUnder(e.getX(), e.getY());
                if(childView != null && gestureDetector.onTouchEvent(e)) {
                    int currentPosition = rv.getChildAdapterPosition(childView);
                    TransactionData currentTD = txEthArray.get(currentPosition);
    ...
    class getTokenTransaction extends AsyncTask<String, Void, ArrayList<TransactionData>> {
        ...
        protected ArrayList<TransactionData> doInBackground(String... strings) {
            ...
            try {
                ...
                if(txEthArray.size() > 0) {
                    txEthArray.clear();
                }
                if(transactionData.length > 0) {
                    for(TransactionData td : transactionData) {
                        if(td.getValue().length() > 6) {
                            txEthArray.add(td);
                        }
                    }
                }
        ...
        protected void onPostExecute(ArrayList<TransactionData> transactionData) {
            ...
            if(txEthArray.size() > 0) {
                txAdapter = new TransactionAdapter(getContext(), txEthArray, walletAddress);
                TxEthRecycle.setAdapter(txAdapter);
            }else {
                noTxEthList.setVisibility(View.VISIBLE);
            }
        }
    }
}
