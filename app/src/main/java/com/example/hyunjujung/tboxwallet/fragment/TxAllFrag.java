...
import com.example.hyunjujung.tboxwallet.WalletMain;
...
public class TxAllFrag extends Fragment{
    private static final String TAG = "TxAllFrag";
    private static final String TX_URL = "http://api-ropsten.etherscan.io/api?module=account&action=txlist&address=";
    ...
    RecyclerView TxAllRecycler;
    LinearLayoutManager txAllLM;
    ArrayList<TransactionData> allTxArray = new ArrayList<>();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.txall_frag, container, false);
        TxAllRecycler = view.findViewById(R.id.TxAllRecycler);
        noTxList = view.findViewById(R.id.noTxList);
        txAllLM = new LinearLayoutManager(getContext());
        TxAllRecycler.setLayoutManager(txAllLM);
        ...
        /* 리사이클러뷰 아이템터치리스너 */
        TxAllRecycler.addOnItemTouchListener(new RecyclerView.OnItemTouchListener() {
            @Override
            public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent e) {
                View childView = rv.findChildViewUnder(e.getX(), e.getY());
                if(childView != null && gestureDetector.onTouchEvent(e)) {
                    int currentPosition = rv.getChildAdapterPosition(childView);
                    TransactionData currentTD = allTxArray.get(currentPosition);
    ...
    class getTokenTransaction extends AsyncTask<String, Void, ArrayList<TransactionData>> {
        ...
        protected void onPostExecute(ArrayList<TransactionData> transactionData) {
            ...
            if(allTxArray.size() > 0) {
                txAdapter = new TransactionAdapter(getContext(), allTxArray, walletAddress);
                TxAllRecycler.setAdapter(txAdapter);
            }else {
                noTxList.setVisibility(View.VISIBLE);
            }
        }
    }
}
